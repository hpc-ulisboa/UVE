...
#include <stdint.h>
#include <stdio.h> //JMNOTE: DEBUG
/* #define MY_DEBUG "JM_DEBUG" */
	...
	break;
      ...
      case 'Y': /*JMNOTE: UVE*/
        switch(c = *p++)
        {
          ...
          case 'n':	USE_BITS (OP_MASK_PN,		OP_SH_PN);	break;
          case 'm':	USE_BITS (OP_MASK_PM,		OP_SH_PM);	break;
          default:
            as_bad(_("internal: bad RISC-V opcode (unknown operand type `Y%c'): %s %s"),
		    c, opc->name, opc->args);
            return FALSE;
        }
        break;
      ...
      case 'k': /*JMNOTE: UVE*/
        used_bits |= ENCODE_UVEATYPE_IMM (-1U); break;
      case 'o':
...
md_begin (void)
{
  ...
  hash_reg_names (RCLASS_UVEPR, riscv_uvepr_names_numeric, NUVEPR);
  hash_reg_names (RCLASS_UVER, riscv_uver_names_numeric, NUVER);
...
riscv_ip(char *str, struct riscv_cl_insn *ip, expressionS *imm_expr,
  ...
  argsStart = s;
    for (; insn && insn->name && strcmp(insn->name, str) == 0; insn++) {
        if (!riscv_subset_supports(insn->subset)) continue;
      ...
      *imm_reloc = BFD_RELOC_UNUSED;
      ...
      debug("Created insn: %s\n", str);

        for (args = insn->args;; ++args) {
            // debug("Inside args for: args = %c\n", *args);
            s += strspn(s, " \t");
            switch (*args) {
                case '\0': /* End of args.  */
                    if (insn->pinfo != INSN_MACRO) {
                        if (!insn->match_func(insn, ip->insn_opcode)) break;
                        if (riscv_insn_length(insn->match) == 2 &&
                            !riscv_opts.rvc)
		    break;
		}
                    if (*s != '\0') break;
		...
		case 's': /* RS1 x8-x15 */
		  ...
		  continue;
                        case 'w': /* RS1 x8-x15, constrained to equal RD
                                     x8-x15.  */
                            if (!reg_lookup(&s, RCLASS_GPR, &regno) ||
                                EXTRACT_OPERAND(CRS1S, ip->insn_opcode) + 8 !=
                                    regno)
		    break;
		  continue;
		case 't': /* RS2 x8-x15 */
		  ...
		  continue;
                        case 'x': /* RS2 x8-x15, constrained to equal RD
                                     x8-x15.  */
                            if (!reg_lookup(&s, RCLASS_GPR, &regno) ||
                                EXTRACT_OPERAND(CRS2S, ip->insn_opcode) + 8 !=
                                    regno)
		    break;
		  continue;
		...
		case 'c': /* RS1, constrained to equal sp.  */
		  ...
		  continue;
		case '>':
                            if (my_getSmallExpression(imm_expr, imm_reloc, s,
                                                      p) ||
		    ...
		    break;
                            ip->insn_opcode |=
                                ENCODE_RVC_IMM(imm_expr->X_add_number);
rvc_imm_done:
		  s = expr_end;
		  imm_expr->X_op = O_absent;
		  continue;
		...
		case 'i':
		    ...
		    break;
                            ip->insn_opcode |=
                                ENCODE_RVC_SIMM3(imm_expr->X_add_number);
		  goto rvc_imm_done;
		case 'j':
                            if (my_getSmallExpression(imm_expr, imm_reloc, s,
                                                      p) ||
		...
		case 'k':
		    ...
		    break;
                            ip->insn_opcode |=
                                ENCODE_RVC_LW_IMM(imm_expr->X_add_number);
		  goto rvc_imm_done;
		case 'l':
                            if (riscv_handle_implicit_zero_offset(imm_expr, s))
		    continue;
		    ...
		    break;
                            ip->insn_opcode |=
                                ENCODE_RVC_LD_IMM(imm_expr->X_add_number);
		  goto rvc_imm_done;
		case 'm':
                            if (riscv_handle_implicit_zero_offset(imm_expr, s))
		    continue;
		...
		case 'o':
                            if (my_getSmallExpression(imm_expr, imm_reloc, s,
                                ...
                                /* C.addiw, c.li, and c.andi allow zero
                                   immediate. C.addi allows zero immediate as
                                   hint.  Otherwise this is same as 'j'.  */
                                || !VALID_RVC_IMM(imm_expr->X_add_number))
		    break;
                            ip->insn_opcode |=
                                ENCODE_RVC_IMM(imm_expr->X_add_number);
		...
		case 'K':
                            if (my_getSmallExpression(imm_expr, imm_reloc, s,
                                ...
                                imm_expr->X_add_number == 0)
		    break;
                            ip->insn_opcode |= ENCODE_RVC_ADDI4SPN_IMM(
                                imm_expr->X_add_number);
		  goto rvc_imm_done;
		case 'L':
                            if (my_getSmallExpression(imm_expr, imm_reloc, s,
                                ...
                                imm_expr->X_add_number == 0)
		    break;
                            ip->insn_opcode |= ENCODE_RVC_ADDI16SP_IMM(
                                imm_expr->X_add_number);
		  goto rvc_imm_done;
		case 'M':
                            if (riscv_handle_implicit_zero_offset(imm_expr, s))
...
rvc_lui:
		...
		case 'v':
		    ...
		    break;
		  imm_expr->X_add_number =
                                ((uint32_t)imm_expr->X_add_number) >>
                                RISCV_IMM_BITS;
		  goto rvc_lui;
		case 'p':
		  goto branch;
		...
		case 'T': /* Floating-point RS2.  */
                            if (!reg_lookup(&s, RCLASS_FPR, &regno)) break;
                            INSERT_OPERAND(CRS2, *ip, regno);
		  continue;
		default:
    ...
    case 'X': /*JMNOTE: UVE */
      ...
      debug("Matched Xd operand: %s\n", s);
                            if (!reg_lookup(&s, RCLASS_UVER, &regno)) break;
                            debug("Inserting operand: regno=%d\n", regno);
        ...
        default:
                            as_bad(_("bad UVE field specifier 'U%c'\n"),
                                   *args);
      }
      break;
    ...
    case 'Y': /*JMNOTE: UVE */
      ...
      debug("Matched Xd operand: %s\n", s);
                            if (!reg_lookup(&s, RCLASS_UVEPR, &regno)) break;
                            debug("Inserting operand: regno=%d\n", regno);
        ...
        case 's': /* PS1 */
                            if (!reg_lookup(&s, RCLASS_UVEPR, &regno)) break;
                            INSERT_OPERAND(PS, *ip, regno);
          continue;

        case 'n': /* PS2 */
                            if (!reg_lookup(&s, RCLASS_UVEPR, &regno)) break;
                            INSERT_OPERAND(PN, *ip, regno);
          continue;

        case 'm': /* PS3 */
                            if (!reg_lookup(&s, RCLASS_UVEPR, &regno)) break;
                            INSERT_OPERAND(PM, *ip, regno);
          continue;
      ...
      case 'v': /*JMNOTE: UVE */
	    ...
	    case ']':
	      ...
	      continue;
                ...
                case 'm': /* Rounding mode.  */
                    if (arg_lookup(&s, riscv_rm, ARRAY_SIZE(riscv_rm),
                                   &regno)) {
	    ...
	    case 'P':
                case 'Q': /* Fence predecessor/successor.  */
                    if (arg_lookup(&s, riscv_pred_succ,
		  ...
		  c = *args;
                        if (*s == ' ') ++s;
                        debug("Match generic register, %s, %d\n", s, regno);
...
md_apply_fix (fixS *fixP, valueT *valP, segT seg ATTRIBUTE_UNUSED)
{
  ...
  switch (fixP->fx_r_type)
      ...
      if (fixP->fx_addsy)
	{
	  ...
	  bfd_vma target = S_GET_VALUE (fixP->fx_addsy) + *valP;
	  bfd_vma delta = target - md_pcrel_from (fixP);
	  bfd_putl32 (bfd_getl32 (buf) | ENCODE_UVEATYPE_IMM (delta), buf);
	}
      break;
