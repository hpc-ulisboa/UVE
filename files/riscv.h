...
// JMNOTE: UVE A TYPE IMM
#define EXTRACT_UVEATYPE_IMM(x)                                             \
    ((RV_X(x, 8, 4) << 1) | (RV_X(x, 22, 6) << 5) | (RV_X(x, 7, 1) << 11) | \
     ((-(RV_X(x, 28, 1))) << 12))
#define ENCODE_UVEATYPE_IMM(x)                                               \
    (((RV_X(x, 1, 4) << 8) | (RV_X(x, 5, 6) << 22) | (RV_X(x, 11, 1) << 7) | \
      (RV_X(x, 12, 1) << 28)))
#define VALID_UVEATYPE_IMM(x) \
    (EXTRACT_UVEATYPE_IMM(ENCODE_UVEATYPE_IMM(x)) == (x))
#define RISCV_UVEATYPE(insn, rs1, target) \
    ((MATCH_##insn) | ((rs1) << OP_SH_RS1) | ENCODE_UVEATYPE_IMM(target))

#define ENCODE_ITYPE_IMM(x) (RV_X(x, 0, 12) << 20)
#define ENCODE_STYPE_IMM(x) ((RV_X(x, 0, 5) << 7) | (RV_X(x, 5, 7) << 25))
#define ENCODE_SBTYPE_IMM(x)                                                \
    ((RV_X(x, 1, 4) << 8) | (RV_X(x, 5, 6) << 25) | (RV_X(x, 11, 1) << 7) | \
     (RV_X(x, 12, 1) << 31))
...
#define ENCODE_RVC_ADDI4SPN_IMM(x)                                         \
    ((RV_X(x, 2, 1) << 6) | (RV_X(x, 3, 1) << 5) | (RV_X(x, 4, 2) << 11) | \
     (RV_X(x, 6, 4) << 7))
#define ENCODE_RVC_ADDI16SP_IMM(x)                                        \
    ((RV_X(x, 4, 1) << 6) | (RV_X(x, 5, 1) << 2) | (RV_X(x, 6, 1) << 5) | \
     (RV_X(x, 7, 2) << 3) | (RV_X(x, 9, 1) << 12))
...
//JMNOTE: UVE Fields
#define OP_MASK_PD 0xF
#define OP_SH_PD 7
#define OP_MASK_PS 0xF
#define OP_SH_PS 15
#define OP_MASK_PM 0x7
#define OP_SH_PM 25
#define OP_MASK_PN 0x7
#define OP_SH_PN 20
...
// JMNOTE: UVE
extern const char * const riscv_uvepr_names_numeric[NUVEPR];
extern const char * const riscv_uver_names_numeric[NUVER];
