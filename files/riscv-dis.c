...
static const char * const *riscv_uvepr_names;
static const char * const *riscv_uver_names;
...
set_default_riscv_dis_options (void)
{
  ...
  riscv_uvepr_names = riscv_uvepr_names_numeric;
  riscv_uver_names = riscv_uver_names_numeric;
  no_aliases = 0;
}
  ...
  for (; *d != '\0'; d++)
    ...
    case 'X': /* JMNOTE: UVE */
	  switch (*++d)
	    ...
	    case 'd': /* VD */
	      print (info->stream, "%s",
		     riscv_uver_names[EXTRACT_OPERAND (RD, l)]);
	      break;
	    case 's': /* RS1 */
	      print (info->stream, "%s",
		     riscv_uver_names[EXTRACT_OPERAND (RS1, l)]);
	      break;
	    case 't': /* RS2 */
	      print (info->stream, "%s",
		     riscv_uver_names[EXTRACT_OPERAND (RS2, l)]);
	      break;
	    }
	  break;
    case 'Y': /* JMNOTE: UVE */
	  switch (*++d)
	    ...
	    case 'd': /* PD */
	      print (info->stream, "%s",
		     riscv_uvepr_names[EXTRACT_OPERAND (PD, l)]);
	      break;
	    case 's': /* PS1 */
	      print (info->stream, "%s",
		     riscv_uvepr_names[EXTRACT_OPERAND (PS, l)]);
	      break;
	    case 'n': /* PS2 */
	      print (info->stream, "%s",
		     riscv_uvepr_names[EXTRACT_OPERAND (PN, l)]);
	      break;
	    case 'm': /* PS2 */
	      print (info->stream, "%s",
		     riscv_uvepr_names[EXTRACT_OPERAND (PM, l)]);
	      break;
			}
	  break;
	case 'v': /*JMNOTE: UVE */
	...
	case 'k':
		info->target = EXTRACT_UVEATYPE_IMM (l) + pc;
		(*info->print_address_func) (info->target, info);
		break;
