...
extern void uve_config(void* src1/*data*/, void*src2/*mean*/, uint64_t sizeN, uint64_t sizeM);
extern ResultType uve_kernel();
#else
extern ResultType core_kernel(void* src1, void* src2,uint64_t sizeN, uint64_t sizeM);
#endif
...
int main( )
{
    ...
    uve_config(src_1, src_2, N, M);
    // t22 = mysecond();
    result = uve_kernel();
  #else // SVE, ARM, RISC-V, x86 code
    result = core_kernel(src_1, src_2, N, M);
  #endif
    M5resetdumpstats();
    t2 = mysecond();
